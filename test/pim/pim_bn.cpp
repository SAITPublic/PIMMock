...
inline float convertH2F(half h_val) {
  return half_float::detail::half2float<float>(h_val);
}
...
int pim_bn_up_to_256KB(bool block, uint64_t input_len) {
  ...
  ret |= load_data(beta.c_str(), (char *)host_beta->data, host_beta->size);
  ret |= load_data(gamma.c_str(), (char *)host_gamma->data, host_gamma->size);
  ret |= load_data(mean.c_str(), (char *)host_mean->data, host_mean->size);
  ret |= load_data(variance.c_str(), (char *)host_variance->data,
                   host_variance->size);
  ...
  PimExecuteBN(device_output, pim_input, host_beta, host_gamma, host_mean,
               host_variance, 1e-5, nullptr, block);
...
TEST(HIPIntegrationTest, PimNRBN1) {
  EXPECT_TRUE(pim_bn_up_to_256KB(true, 1 * 1024) == 0);
}
TEST(HIPIntegrationTest, PimNRBN2) {
  EXPECT_TRUE(pim_bn_up_to_256KB(true, 64 * 1024) == 0);
}
TEST(HIPIntegrationTest, PimNRBN3) {
  EXPECT_TRUE(pim_bn_up_to_256KB(true, 128 * 1024) == 0);
}
