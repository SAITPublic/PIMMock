...
/* __PIM_API__ call : Execute PIM kernel (ELT_MUL) */
  ...
  //    ret = compare_data((char*)golden_output->data, (char*)host_output->data,
  //    host_output->size);
  ret |= compare_half_relative((half *)golden_output->data,
...
TEST(HIPIntegrationTest, PimEltMul1Sync) {
  EXPECT_TRUE(pim_elt_mul_up_to_512KB(true, 1 * 1024) == 0);
}
TEST(HIPIntegrationTest, PimEltMul1Async) {
  EXPECT_TRUE(pim_elt_mul_up_to_512KB(false, 1 * 1024) == 0);
}
TEST(HIPIntegrationTest, PimEltMul2Sync) {
  EXPECT_TRUE(pim_elt_mul_up_to_512KB(true, 128 * 1024) == 0);
}
TEST(HIPIntegrationTest, PimEltMul2Async) {
  EXPECT_TRUE(pim_elt_mul_up_to_512KB(false, 128 * 1024) == 0);
}
TEST(HIPIntegrationTest, PimEltMul3Sync) {
  EXPECT_TRUE(pim_elt_mul_up_to_512KB(true, 256 * 1024) == 0);
}
TEST(HIPIntegrationTest, PimEltMul3Async) {
  EXPECT_TRUE(pim_elt_mul_up_to_512KB(false, 256 * 1024) == 0);
}
TEST(HIPIntegrationTest, PimEltMul4Sync) {
  EXPECT_TRUE(pim_elt_mul_up_to_512KB(true, 128 * 768) == 0);
}
TEST(HIPIntegrationTest, PimEltMul4Async) {
  EXPECT_TRUE(pim_elt_mul_up_to_512KB(false, 128 * 768) == 0);
}
TEST(HIPIntegrationTest, PimEltMulProfileSync) {
  EXPECT_TRUE(pim_elt_mul_profile(true, 128 * 1024) == 0);
}
