 ...
 * @param PimPrecision  Options to choose PIM operations precision ( PIM_FP16,
 * PIM_INT8)
 *
 ...
 */
__PIM_API__ PimBo *PimCreateBo(int w, int h, int c, int n,
                               PimPrecision precision, PimMemType mem_type,
 ...
 * @param mem_flag Describes operation for which buffer is used for( element
 * wise or gemv)
 * @param user_ptr external memory passed by user. If passed, Bo is created with
 * user pointer. if nullptr, pim library does the allocation
 * @return Pointer to buffer object
 */
__PIM_API__ PimBo *PimCreateBo(PimDesc *pim_desc, PimMemType mem_type,
                               PimMemFlag mem_flag = ELT_OP,
                               void *user_ptr = nullptr);
 ...
 * @return Return success/failure
 */
__PIM_API__ PimDesc *PimCreateDesc(int n, int c, int h, int w,
                                   PimPrecision precision,
                                   PimOpType op_type = OP_ELT_ADD);
 ...
 * @param cpy_type type of memory transfer (HOST to GPU, GPU to HOST, GPU to PIM
 * etc)
 *
 * @return
 */
__PIM_API__ int PimCopyMemory(void *dst, void *src, size_t size,
                              PimMemCpyType cpy_type);
 ...
 *
 * @param copyParams Parameters for the rectangular copy.
 * @return success/failure
 */
__PIM_API__ int PimCopyMemoryRect(const PimCopy3D *copyParams);
 ...
 * @return success/failure
 */
__PIM_API__ int PimExecuteAdd(PimBo *output, PimBo *operand0, PimBo *operand1,
 ...
 * @return success/failure
 */
__PIM_API__ int PimExecuteAdd(PimBo *output, void *scalar, PimBo *vector,
                              void *stream = nullptr, bool block = false);
 ...
 * @return
 */
__PIM_API__ int PimExecuteMul(PimBo *output, PimBo *operand0, PimBo *operand1,
                              void *stream = nullptr, bool block = false);
 ...
 * @return success/failure
 */
__PIM_API__ int PimExecuteMul(PimBo *output, void *scalar, PimBo *vector,
                              void *stream = nullptr, bool block = false);
 ...
 * @return success/failure
 */
__PIM_API__ int PimExecuteRelu(PimBo *output, PimBo *pim_data,
                               void *stream = nullptr, bool block = false);
 ...
 * For PIM GemV operations, weights(kernel values) need to be preprocessed with
 Convert Data PIM API
 * Output values are placed in PIM area and need to be transfered to GPU or HOST
 memory as per requirements
 *
...
__PIM_API__ int PimExecuteGemv(PimBo *output, PimBo *operand0, PimBo *operand1,
                               void *stream = nullptr, bool block = false);
 ...
 * @return success or failure
 */
__PIM_API__ int PimExecuteGemvAdd(PimBo *output, PimBo *operand0,
                                  PimBo *operand1, void *stream = nullptr,
                                  bool block = false);
 ...
 */
__PIM_API__ int PimExecuteGemvAdd(PimBo *output, PimBo *operand0,
                                  PimBo *operand1, PimBo *operand2, bool relu,
                                  void *stream = nullptr, bool block = false);
 ...
 * For PIM GemV list operations, weights(kernel values) need to be preprocessed
 * with Convert Data PIM API Output values are placed in PIM area and need to be
 * transfered to GPU or HOST memory as per requirements
 *
 ...
 * @return success or failure
 */
__PIM_API__ int PimExecuteGemvList(PimBo *output, PimBo *vector, PimBo *matrix,
                                   void *stream = nullptr, bool block = false);
 ...
 * @return success/failure
 */
__PIM_API__ int PimExecuteBN(PimBo *output, PimBo *pim_data, PimBo *beta,
                             PimBo *gamma, PimBo *mean, PimBo *variance,
                             double epsilon, void *stream = nullptr,
                             bool block = false);
